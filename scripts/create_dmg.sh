...
if [[ ! -f "$INSTALLER_PATH" ]]; then
  echo "Missing installer helper at $INSTALLER_PATH"
  exit 1
fi
...
/usr/bin/ditto --noextattr --norsrc "$INSTALLER_PATH" "$DMG_STAGING/$INSTALLER_NAME"
chmod +x "$DMG_STAGING/$INSTALLER_NAME"
